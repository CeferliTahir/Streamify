  ...
  return (
    <>
      <div className="flex flex-col gap-2 cursor-pointer">
        <div className="md:w-[280px] md:h-[174px] w-[100%] h-[110px] sm:h-[140px] rounded-lg relative overflow-hidden group">
          <img
            className="rounded-lg text-white w-full h-full object-cover"
            src={import.meta.env.VITE_IMAGE_BASE_URL + img}
            alt="SORRY"
          />
          <button className="save-button absolute top-4 right-4 z-10">
            <svg
        ...
        <div className=" text-white font-abc md:w-[280px] flex flex-col gap-[5px]">
          <div className="flex items-center md:text-[13px] text-[11px] sm:text-[13px] font-normal opacity-[0.75] gap-2">
            <p>
            ...
            <div className="flex items-center gap-[6px]">
              ...
              <p>{name ? "TV Series" : "Movies"}</p>
            </div>
            <p>.</p>
            <p>{originalLanguage}</p>
          </div>
          <h1 className="md:text-lg text-sm sm:text-lg font-normal">
            {title} {name}
          </h1>
        </div>
      </div>
